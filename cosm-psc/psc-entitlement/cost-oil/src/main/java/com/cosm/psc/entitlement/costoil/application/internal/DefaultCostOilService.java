...
public class DefaultCostOilService implements CostOilService {
	...
	public void when(CostOilReady event) {
		...
		CostOilProjection prevCostOilProj = costOilRepository.costOilProjectionOfFiscalPeriod(prevFp, new ProductionSharingContractId(event.getPscId()));
		...
		.withPriorCostOilProjection(Optional.of(prevCostOilProj));
