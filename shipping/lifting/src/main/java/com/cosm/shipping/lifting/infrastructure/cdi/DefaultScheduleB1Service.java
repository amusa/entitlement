package com.cosm.shipping.lifting.infrastructure.cdi;
...
import com.cosm.shipping.lifting.domain.service.ScheduleB1Service;
 ...
 */
@Dependent
public class DefaultScheduleB1Service implements ScheduleB1Service {
    ...
    @Override
    public List<ProceedAllocation> computeProceedAllocation(ProductionSharingContractId pscId, int year, int month) {
    ...
    @Override
    public List<LiftingSummary> getLiftingSummary(ProductionSharingContractId pscId, int year, int month) {
        List<LiftingSummary> liftingSummaryList = new ArrayList<>();

        if (pscId != null && year != 0 && month != 0) {
            List<Lifting> liftings = liftingBean.find(pscId, year, month);

            double proceed = 0;

            for (Lifting lifting : liftings) {
