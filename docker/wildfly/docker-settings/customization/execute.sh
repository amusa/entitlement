...
#set CONNECTION_URL=jdbc:mysql://$MYSQL_SERVICE_SERVICE_HOST:$MYSQL_SERVICE_SERVICE_PORT/entitlement
set CONNECTION_URL=jdbc:mysql://$MYSQL_URI/entitlement
echo "Connection URL: " $CONNECTION_URL

# Execute the batch
#run-batch --file=/opt/jboss/wildfly/customization/add-datasource.sh
# Add MySQL module
...
run-batch --file=/opt/jboss/wildfly/customization/add-cosm-domain.sh
EOF



# Deploy the WAR
cp /opt/jboss/wildfly/customization/cosm-entitlement-1.0-SNAPSHOT.war $JBOSS_HOME/$JBOSS_MODE/deployments/cosm-entitlement-1.0-SNAPSHOT.war
